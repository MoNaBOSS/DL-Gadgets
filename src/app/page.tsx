...
export default async function Home() {
  ...
  return <>
    <section className="border-b border-slate-200 bg-slate-50">
      <Container className="grid gap-8 py-10 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:py-16">
        <div>
          ...
          <p className="mt-4 max-w-xl text-base leading-7 text-slate-600">Phones, tablets, gaming, security, and everyday tech. Add in-stock products with a confirmed price to cart, or request a quote for stock that needs a personal check.</p>
        ...
        <div className="relative min-h-64 overflow-hidden rounded-2xl border border-slate-200 bg-white sm:min-h-80"><Image src="/products/electronics-collection.webp" alt="DL Gadgets electronics collection" fill preload sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover" /></div>
      </Container>
    </section>
    ...
    <section className="bg-slate-950 py-12 text-white"><Container><div className="grid gap-7 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-300">How ordering works</p><h2 className="mt-3 text-3xl font-black">Choose checkout or a personal quote.</h2><p className="mt-3 leading-7 text-slate-300">Confirmed-price stock can be paid securely online. Request-price products stay personal so we can confirm the details first.</p></div><ol className="grid gap-3 sm:grid-cols-2">{[["01", "Browse", "Explore product listings."], ["02", "Add or request", "Add priced items to cart or request a quote."], ["03", "Pay or confirm", "Use secure checkout or receive our confirmation."], ["04", "Collect / deliver", "Agree the practical next step."]].map(([number, title, copy]) => <li key={number} className="rounded-xl border border-white/10 bg-white/5 p-4"><span className="text-sm font-black text-cyan-300">{number}</span><h3 className="mt-3 font-black">{title}</h3><p className="mt-1 text-sm text-slate-300">{copy}</p></li>)}</ol></div></Container></section>
